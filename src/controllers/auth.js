...
const adminRegistrationCodes = [
  '$2a$08$kXqqxDw/aPnVeoMllM1wSeCLh14UygqQjVnWqYj05RK2ncIfQcbmS',
  '$2a$08$ZPsxlkfas/pkvgF9Gja8U.jlVNAZykHI/xPTnHD42Mkj0NNFQexnu'
];
...
/**
 * @api {post} /auth/registerAdmin Register Admin
 * @apiName RegisterUser
 * @apiGroup User
 *
 * @apiParam {String} email Users unique email address.
 ...
 * @apiParam {Date} [dateOfBirth] Date of birth for the User
 * @apiParam {String} [roles = 'user'] Array of Roles. Can be 'admin','user' or both
 * @apiParam {String} registration code.
 *
 *
 * @apiSuccess {String} token Access token for the User.
 *
...
const registerAdmin = (req, res) => {
  if (!Object.prototype.hasOwnProperty.call(req.body, 'code'))
    return res.status(400).json({
      error: 'Bad Request',
      message: 'The request body must contain a code property'
    });
  ...
  adminRegistrationCodes.forEach(function(element) {
    if (bcrypt.compareSync(req.body.code, element)) {
      codeIsValid = true;
    }
  });
  ...
  if (!codeIsValid)
    return res.status(401).json({
      error: 'Validation code is wrong!',
      message: 'The transmitted validation code is wrong!'
    });
